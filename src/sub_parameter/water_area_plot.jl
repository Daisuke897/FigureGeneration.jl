...
struct Plot_core_water_area end

Plots.RecipesBase.@recipe function f(
    ::Plot_core_water_area,
    japanese::Bool,
...
function make_graph_water_area(
    df_main::Main_df,
    time_schedule,
    target_hour::Int,
...
function make_graph_water_area_yearly_mean(
    df_main::Main_df,
    each_year_timing::Each_year_timing,
    year_first::Int,
    year_last::Int,    
    ...
    Plots.plot!(
        ...
        title = if japanese == true
            string("水 断面積 ", year_first, " - ", year_last)
        else
            string("Water area ", year_first, " - ", year_last)
        end
    )
