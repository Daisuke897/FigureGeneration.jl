...
struct Plot_core_water_velocity end

Plots.RecipesBase.@recipe function f(
    ::Plot_core_water_velocity,
    japanese::Bool,
...
struct Plot_water_velocity end
...
function make_graph_water_velocity(
    df_main::Main_df,
    time_schedule,
    target_hour::Int,
...
function make_graph_water_velocity_yearly_mean(
    df_main::Main_df,
    each_year_timing::Each_year_timing,
    year_first::Int,
    year_last::Int,    
    ...
    Plots.plot!(
        ...
        title = if japanese == true
            string("流速 ", year_first, " - ", year_last)
        else
            string("Water velocity ", year_first, " - ", year_last)
        end
    )
